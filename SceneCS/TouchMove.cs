...
public class TouchMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.touchCount == 1) //单点触碰移动摄像机
        {
            ...
            {
                transform.Translate(new Vector3(-Input.touches[0].deltaPosition.x * Time.deltaTime * MoveSpeed / 100, -Input.touches[0].deltaPosition.y * Time.deltaTime * MoveSpeed / 100, 0));
            }
        }
        ...
        else if (Input.touchCount > 1)//多点触碰
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                //移动摄像机
                Camera.main.transform.Translate(isforward * Vector3.forward * Time.deltaTime * ToteSpeed / 10 * (Mathf.Abs(deltaDis2.x + deltaDis1.x) + Mathf.Abs(deltaDis1.y + deltaDis2.y)));
            }
        }
    }
